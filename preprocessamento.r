...
#Carregando dataset
data <- read.table("MinDadosR/loja.csv",header = TRUE, sep=",")
...
#Vamos visualizar o atributo salario
...
data$capacidade <- cut(data$capacidade, c(34.9, 82.5, 100), labels=c("pequena", "alta"))
#Transformacao nominal para numerico
formacao_codes[data$formacao=="medio"] <- 1
formacao_codes[data$formacao=="superior"] <- 2
formacao_codes[data$formacao=="pos"] <- 3
...
data_novo <- cbind(data$Idade, data$salario, formacao_codes, 
